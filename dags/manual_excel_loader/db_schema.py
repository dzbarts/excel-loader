...
def get_table_columns(
    ...
    """
    Запрашивает имена и типы колонок из целевой таблицы.

    Возвращает dict {col_name: type_string} или None если таблица не найдена.
    ...
    if db_type == DatabaseType.POSTGRES:
        return _pg_columns(scheme, table)
...
def _pg_columns(scheme: str, table: str) -> dict[str, str] | None:
    ...
    if not rows:
        log.debug("PG: table %s.%s not found in information_schema", scheme, table)
        return None
    return {col: type_str for col, type_str in rows}
