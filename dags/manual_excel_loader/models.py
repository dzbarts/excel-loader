...
class LoaderConfig:
    ...
    Поля прогресса:
        show_progress — показывать tqdm прогресс-бар при ручном запуске.
            Оставить False (по умолчанию) при запуске через Airflow.
    """
    ...
    # Директория для выходного SQL/CSV файла (to_sql / to_csv).
    ...
    # Готовый текстовый поток для записи SQL/CSV-файла.
    # Если задан — load() пишет в него напрямую, локальный файл не создаётся.
    # output_dir при этом игнорируется; LoadResult.output_file будет None.
    # Управление жизнью потока (открытие, закрытие) — ответственность вызывающего кода.
    output_stream: IO[str] | None = field(default=None, repr=False, compare=False)
