...
def prepare(
    ...
    if db_type in (DatabaseType.GREENPLUM, DatabaseType.POSTGRES):
        return _prepare_psql(scheme, table, export_mode, create_ddl, db_type)
...
def finalize(context: dict, success: bool) -> None:
    ...
    if db_type in (DatabaseType.GREENPLUM, DatabaseType.POSTGRES):
        _finalize_psql(context, success)
    else:
        _finalize_ch(context, success)


# ── GreenPlum / PostgreSQL (psycopg2-совместимые) ─────────────────────────────
...
def _prepare_psql(
    ...
    from ._connections import get_gp_conn, get_pg_conn

    conn = get_gp_conn() if db_type == DatabaseType.GREENPLUM else get_pg_conn()
    tag = "GP" if db_type == DatabaseType.GREENPLUM else "PG"
    ctx: dict = {
        "db_type": db_type,
        "conn": conn,
        "scheme": scheme,
    ...
    if export_mode == "append":
        ...
        ctx["cursor"] = cur
        log.info("%s: TRUNCATE %s.%s — ожидаем завершения загрузки", tag, scheme, table)
        ...
        with conn.cursor() as cur:
            ...
            if cur.fetchone():
                ...
                log.info("%s: renamed %s.%s → %s.%s", tag, scheme, table, scheme, backup)
                ctx["backup_table"] = backup
            else:
                log.info("%s: таблица %s.%s не существует, backup пропущен", tag, scheme, table)
...
def _psql_create_if_not_exists(conn, ddl: str, scheme: str, table: str, tag: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            "SELECT 1 FROM information_schema.tables "
            "WHERE table_schema=%s AND table_name=%s",
        ...
        if not cur.fetchone():
            cur.execute(ddl)
            conn.commit()
            log.info("%s: создана таблица %s.%s", tag, scheme, table)


def _finalize_psql(ctx: dict, success: bool) -> None:
    conn = ctx.get("conn")
    scheme, table = ctx["scheme"], ctx["table"]
    export_mode = ctx["export_mode"]
    tag = "GP" if ctx["db_type"] == DatabaseType.GREENPLUM else "PG"
    ...
    try:
        if export_mode == "truncate_load":
            ...
            if success:
                conn.commit()
                log.info("%s: truncate_load зафиксирован для %s.%s", tag, scheme, table)
            else:
                conn.rollback()
                log.warning("%s: truncate_load откатан для %s.%s", tag, scheme, table)
            if cur:
                cur.close()
            ...
            if backup:
                ...
                conn.commit()
                log.warning(
                    "%s: via_backup откатан, восстановлено из %s.%s", tag, scheme, backup
                )
    finally:
        if conn:
            conn.close()
