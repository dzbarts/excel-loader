...
def _extract_columns_body(ddl: str) -> str:
    ...
    start = ddl.find("(")
    if start == -1:
        raise ConfigurationError("DDL не содержит скобок — это валидный CREATE TABLE?")
    ...
    raise ConfigurationError("DDL содержит анбалансированные скобки.")
...
_PARSERS: dict[DatabaseType, Callable[[str], dict[str, str]]] = {
    ...
    DatabaseType.POSTGRES: _parse_ddl_gp,
...
def parse_ddl(ddl: str, db_type: DatabaseType) -> dict[str, str]:
    ...
    if not result:
        raise ConfigurationError(
            "DDL распарсился, но не получилось определить колоноки. "
            "Проверь, что это валидный CREATE TABLE стейтмент."
        )
