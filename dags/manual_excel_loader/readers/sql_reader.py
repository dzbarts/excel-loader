...
def read_sql(config: SqlReadConfig) -> SqlSheetData:
    ...
    if config.stream is not None:
        log.info("Читаем SQL из потока (%d байт)", len(config.stream))
        content = config.stream.decode(config.encoding)
    else:
        path = config.path
        if not path.exists():
    ...
    if not tables:
        raise ValueError(f"В файле {config.path} не найдено INSERT-выражений.")
